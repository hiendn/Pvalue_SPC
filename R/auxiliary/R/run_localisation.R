# Fast directional and coordinate localisation simulations.
#
# Run from R/auxiliary:
#   Rscript R/run_localisation.R
#
# Main speedups:
#   * Normal samples are generated by a Cholesky sampler rather than MASS::mvrnorm().
#   * Cauchy samples are generated as multivariate t_1 using a cached Cholesky factor.
#   * Mann--Whitney one-sided exact p-values use precomputed Wilcoxon null CDFs
#     for each possible (n0, nt), avoiding repeated wilcox.test() calls.
#   * Each configuration prints progress and writes partial checkpoint CSV files.
...
for (i in seq_len(nrow(normal_grid))) {
  g <- normal_grid[i, ]
  cp <- sprintf("output/checkpoints/localisation_normal_delta%s_rho%s_alpha%s_raw.csv",
                gsub("\\.", "p", as.character(g$delta)),
                gsub("\\.", "p", as.character(g$rho)),
                gsub("\\.", "p", as.character(g$alpha)))
...
for (i in seq_len(nrow(cauchy_grid))) {
  g <- cauchy_grid[i, ]
  cp <- sprintf("output/checkpoints/localisation_cauchy_n0%d_delta%s_rho%s_alpha%s_raw.csv",
                g$n0,
                gsub("\\.", "p", as.character(g$delta)),
                gsub("\\.", "p", as.character(g$rho)),
                gsub("\\.", "p", as.character(g$alpha)))
...
log_msg("Finished fast localisation simulations.", verbose = VERBOSE)
